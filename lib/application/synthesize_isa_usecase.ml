...
module Make (Entropy : Entropy_port.S) = struct
  ...
  let synthesize_4vcpu ?(name : string option)
      ?(params : C_isa_sail_templates.synth_params = C_isa_sail_templates.default_params)
      ~(out_dir : string) () : unit =
    Synth.synthesize_4vcpu_cascade ?name ~params ~out_dir ()

  let synthesize_8vcpu
      ?(params : C_isa_sail_templates.synth_params = C_isa_sail_templates.default_params)
      ~(out_dir : string) () : unit =
    Synth.synthesize_8vcpu_cascade ~params ~out_dir ()
  ...
  let synthesize_single ~(vcpu : string) ~(out_json : string) ?(out_sail : string option) ?(name : string option)
    ...
    Synth.synthesize_single ~vcpu ~out_json ?out_sail ?name ~params ()
end
