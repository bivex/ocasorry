...
def extract_ast_features(expr):
    ...
    while stack:
        ...
        try:
            ...
            elif kind in (z3.Z3_OP_BSHL, z3.Z3_OP_BLSHR, z3.Z3_OP_BASHR): counts["shift"] += 1
        except Exception:
            pass
...
class SMTAttackEnv:
    ...
    Strict handling: unsat = verified, sat = bug (-100), unknown/timeout = unproven (-50).
    """
    ACTION_NAMES = [
        "MBA_DECOMP_ADD",        # x + y <=> (x ^ y) + 2*(x & y)
        "MBA_DECOMP_SUB",        # x - y <=> (x ^ y) - 2*(~x & y)
        "MBA_AFFINE_XOR",        # x ^ y <=> (x | y) - (x & y)
        "MBA_DEMORGAN_OR",       # x | y <=> (x ^ y) + (x & y)
        "QUADRATIC_INVARIANT",   # (a * (a + 1)) & 1 == 0 in Z_{2^64}
        "AFFINE_SBOX_ENTANGLE",  # Reversible affine layer in Z_{2^64}
        "DECOY_JUMP_GUARD",      # Accumulator perturbation
        "REGISTER_ROTATION_SEED" # VREG rotation mapping
    ]

    # 64-Bit Affine Constants (k1 * inv_k1 == 1 mod 2^64)
    K1_64 = 0x9E3779B97F4A7C15
    INV_K1_64 = 0xF1DE83E19937733D
    K2_64 = 0x517CC1B727220A95

    def __init__(self, bit_width=64):
    ...
    def reset(self, target_op="ADD"):
        ...
        if target_op == "ADD": self.orig_expr = self.a_sym + self.b_sym
        elif target_op == "SUB": self.orig_expr = self.a_sym - self.b_sym
        elif target_op == "XOR": self.orig_expr = self.a_sym ^ self.b_sym
        elif target_op == "OR": self.orig_expr = self.a_sym | self.b_sym
        elif target_op == "AND": self.orig_expr = self.a_sym & self.b_sym
        else: self.orig_expr = self.a_sym + self.b_sym
    ...
    def get_state(self):
        """Returns 12-dimensional numerical feature tensor from AST."""
    ...
    def step(self, action_idx):
        ...
        if action == "MBA_DECOMP_ADD":
            ...
            self.c_code_steps.append(("MBA_DECOMP_ADD", "__cur = (__cur ^ __b) + ((__cur & __b) << 1ULL) - __b;"))
        elif action == "MBA_DECOMP_SUB":
            self.current_expr = (self.current_expr ^ b) - ((~self.current_expr & b) << 1) + b
            self.c_code_steps.append(("MBA_DECOMP_SUB", "__cur = (__cur ^ __b) - ((~__cur & __b) << 1ULL) + __b;"))
        elif action == "MBA_AFFINE_XOR":
            self.current_expr = (self.current_expr | b) - (self.current_expr & b) - b + (a ^ b)
            self.c_code_steps.append(("MBA_AFFINE_XOR", "__cur = ((__cur | __b) - (__cur & __b)) - __b + (__a ^ __b);"))
        elif action == "MBA_DEMORGAN_OR":
            self.current_expr = (self.current_expr ^ b) + (self.current_expr & b) - b
            self.c_code_steps.append(("MBA_DEMORGAN_OR", "__cur = (__cur ^ __b) + (__cur & __b) - __b;"))
        elif action == "QUADRATIC_INVARIANT":
            inv = (a * (a + 1)) & 1
            self.current_expr = self.current_expr ^ inv
            self.c_code_steps.append(("QUADRATIC_INVARIANT", "__cur ^= ((__a * (__a + 1ULL)) & 1ULL);"))
            ...
            self.c_code_steps.append(("AFFINE_SBOX_ENTANGLE", f"__cur = (((__cur + 0x{self.K2_64:016X}ULL) * 0x{self.K1_64:016X}ULL) * 0x{self.INV_K1_64:016X}ULL) - 0x{self.K2_64:016X}ULL;"))
        elif action == "DECOY_JUMP_GUARD":
            self.c_code_steps.append(("DECOY_JUMP_GUARD", "__vm_state_acc = (__vm_state_acc * 0x63c63cd93839c9b9ULL) ^ 0x517CC1B727220A95ULL;"))
        elif action == "REGISTER_ROTATION_SEED":
            self.c_code_steps.append(("REGISTER_ROTATION_SEED", "/* Dynamic Register Scramble Seed Shift */"))
        ...
        return self.get_state(), reward, done, {"valid": is_valid, "status": str(res), "z3_time": solve_duration}
