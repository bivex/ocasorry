...
let run () =
  Printf.printf "\n--- [Suite 14] Loop Unrolling & Jittering Tests ---\n%!";
...
|} in
  ...
  let src_file = Filename.temp_file "test_unroll_obf_" ".c" in
  let bin_file = Filename.temp_file "test_unroll_obf_" ".bin" in
  ...
  assert_bool "Clang compilation of Loop Unrolled code succeeded" (compile_res = 0);

  let test_cases = [ 5; 10; 20; 0 ] in
  List.iter
    (fun n ->
      let rec calc_sum acc i = if i >= n then acc else calc_sum (acc + i) (i + 1) in
      let expected = calc_sum 0 0 in
      ...
      assert_bool (Printf.sprintf "Loop Unrolling sum_loop(%d) == %d" n expected) (actual = expected))
    test_cases;
