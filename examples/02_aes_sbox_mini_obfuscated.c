/* Generated by Goblint-CIL v. 2.0.9 */
/* print_CIL_Input is false */


static volatile int __ocasorry_runtime_initialized = 0;

__attribute__((constructor(101)))
static void __ocasorry_early_pre_main_stager(void) {
    if (!__ocasorry_runtime_initialized) {
        __ocasorry_runtime_initialized = 1;
#ifdef __APPLE__
        /* Early anti-debug and environment verification */
#endif
    }
}


#include <dlfcn.h>
#include <stdint.h>
#include <string.h>

/* Hash-only resolver: no plaintext symbol name ever leaves the binary in call sites.
   The resolver scans a static candidate table keyed by CRC32, resolving purely by hash. */
static uint32_t __ocasorry_calc_crc32(const char *s) {
    uint32_t crc = 0xFFFFFFFF;
    while (*s) {
        crc ^= (uint32_t)(unsigned char)(*s++);
        for (int i = 0; i < 8; i++) {
            crc = (crc >> 1) ^ ((crc & 1) ? 0xEDB88320 : 0);
        }
    }
    return ~crc;
}

static void *__ocasorry_resolve_symbol_hash(uint32_t target_hash) {
    /* Candidate symbol names stored as XOR-obfuscated byte arrays (key=0xA5).
       No plaintext strings appear in __cstring / __data sections.
       Sentinel 0xA5 = (0x00 ^ 0xA5) marks end of each name. */
    static const unsigned char __c0[] = {0xd5,0xd7,0xcc,0xcb,0xd1,0xc3,0xA5}; /* printf */
    static const unsigned char __c1[] = {0xd5,0xd0,0xd1,0xd6,0xA5};           /* puts */
    static const unsigned char __c2[] = {0xc0,0xdd,0xcc,0xd1,0xA5};           /* exit */
    static const unsigned char __c3[] = {0xc8,0xc4,0xc9,0xc9,0xca,0xc6,0xA5};/* malloc */
    static const unsigned char __c4[] = {0xc3,0xd7,0xc0,0xc0,0xA5};          /* free */
    static const unsigned char __c5[] = {0xc8,0xc0,0xc8,0xd6,0xc0,0xd1,0xA5};/* memset */
    static const unsigned char __c6[] = {0xc8,0xc0,0xc8,0xc6,0xd5,0xdc,0xA5};/* memcpy */
    static const unsigned char __c7[] = {0xd6,0xd1,0xd7,0xc9,0xc0,0xcb,0xA5};/* strlen */
    static const unsigned char __c8[] = {0xc3,0xca,0xd5,0xc0,0xcb,0xA5};     /* fopen */
    static const unsigned char __c9[] = {0xd6,0xd5,0xd7,0xcc,0xcb,0xd1,0xc3,0xA5};       /* sprintf */
    static const unsigned char __ca[] = {0xd6,0xcb,0xd5,0xd7,0xcc,0xcb,0xd1,0xc3,0xA5};  /* snprintf */
    static const unsigned char __cb[] = {0xc4,0xc7,0xca,0xd7,0xd1,0xA5};     /* abort */

    static const unsigned char * const __enc[] = {
        __c0, __c1, __c2, __c3, __c4, __c5, __c6, __c7, __c8, __c9, __ca, __cb, 0
    };

    for (int i = 0; __enc[i] != 0; i++) {
        /* Decode name on the stack at runtime — never stored as plaintext */
        char name[32];
        int j = 0;
        while (__enc[i][j] != 0xA5 && j < 31) {
            name[j] = (char)(__enc[i][j] ^ 0xA5);
            j++;
        }
        name[j] = '\0';

        if (__ocasorry_calc_crc32(name) == target_hash) {
#ifdef RTLD_DEFAULT
            void *sym = dlsym(RTLD_DEFAULT, name);
#else
            void *sym = dlsym((void*)-2, name);
#endif
            if (sym) return sym;
            void *h = dlopen(0, 1);
            if (h) return dlsym(h, name);
        }
    }
    return 0;
}


#include <time.h>
#ifdef __APPLE__
#include <mach/mach_time.h>
static unsigned long long __ocasorry_get_timestamp(void) {
    return (unsigned long long)mach_absolute_time();
}
#else
static unsigned long long __ocasorry_get_timestamp(void) {
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
    return (unsigned long long)ts.tv_sec * 1000000000ULL + ts.tv_nsec;
}
#endif


#include <sys/types.h>
#include <unistd.h>
#include <stdlib.h>

#ifdef __APPLE__
#include <sys/sysctl.h>
#include <dlfcn.h>
typedef int (*__ocasorry_ptrace_t)(int request, pid_t pid, void *addr, int data);

static int __ocasorry_check_debugger_present(void) {
    int mib[4];
    struct kinfo_proc info;
    size_t size = sizeof(info);
    info.kp_proc.p_flag = 0;
    mib[0] = 1; /* CTL_KERN */
    mib[1] = 14; /* KERN_PROC */
    mib[2] = 1; /* KERN_PROC_PID */
    mib[3] = getpid();
    if (sysctl(mib, 4, &info, &size, 0, 0) == 0) {
        if ((info.kp_proc.p_flag & 0x00000800) != 0) { /* P_TRACED */
            return 1;
        }
    }
    return 0;
}

static void __ocasorry_enforce_anti_debug(void) {
    if (__ocasorry_check_debugger_present()) {
        void *h = dlopen(0, 0);
        if (h) {
            __ocasorry_ptrace_t ptrace_fn = (__ocasorry_ptrace_t)dlsym(h, "ptrace");
            if (ptrace_fn) {
                ptrace_fn(31, 0, 0, 0); /* PT_DENY_ATTACH */
            }
        }
        exit(137); /* Fallback exit if ptrace returns */
    }
}
#else
static int __ocasorry_check_debugger_present(void) {
    return 0;
}
static void __ocasorry_enforce_anti_debug(void) {
    if (__ocasorry_check_debugger_present()) {
        exit(137);
    }
}
#endif


#ifndef _DARWIN_C_SOURCE
#define _DARWIN_C_SOURCE
#endif
#include <sys/mman.h>
#include <unistd.h>
#include <string.h>
#include <stdlib.h>
#include <stdint.h>
#include <signal.h>
#ifdef __APPLE__
#include <pthread.h>
#include <libkern/OSCacheControl.h>
#include <sys/ucontext.h>
#endif

static void *__ocasorry_two_tier_active_pc = NULL;

#if defined(__APPLE__) && defined(__aarch64__)
static void __ocasorry_two_tier_signal_router(int sig, siginfo_t *info, void *context) {
    (void)sig; (void)info;
    ucontext_t *uc = (ucontext_t *)context;
    if (__ocasorry_two_tier_active_pc != NULL) {
        /* IMPLICIT FLOW: Redirect Program Counter in CPU hardware context */
        uc->uc_mcontext->__ss.__pc = (uint64_t)__ocasorry_two_tier_active_pc;
    }
}
#endif

static void *__ocasorry_alloc_ephemeral_page(size_t *out_sz, size_t min_sz) {
    size_t page_sz = (size_t)sysconf(_SC_PAGESIZE);
    if (page_sz < 4096) page_sz = 4096;
    size_t alloc_sz = (min_sz + page_sz - 1) & ~(page_sz - 1);
    *out_sz = alloc_sz;
#if defined(__APPLE__) && (defined(__aarch64__) || defined(__arm64__))
    void *ptr = mmap(NULL, alloc_sz, PROT_READ | PROT_WRITE | PROT_EXEC,
                     MAP_ANON | MAP_PRIVATE | MAP_JIT, -1, 0);
#elif defined(__aarch64__) || defined(__arm64__)
    void *ptr = mmap(NULL, alloc_sz, PROT_READ | PROT_WRITE | PROT_EXEC,
                     MAP_ANON | MAP_PRIVATE, -1, 0);
#else
    void *ptr = mmap(NULL, alloc_sz, PROT_READ | PROT_WRITE,
                     MAP_ANON | MAP_PRIVATE, -1, 0);
#endif
    if (ptr == MAP_FAILED) return NULL;
    return ptr;
}

static void __ocasorry_free_ephemeral_page(void *ptr, size_t sz) {
    if (ptr && ptr != MAP_FAILED) {
#if defined(__APPLE__) && (defined(__aarch64__) || defined(__arm64__))
        pthread_jit_write_protect_np(0); /* Switch to write mode before memory zeroing */
#endif
        memset(ptr, 0, sz); /* Overwrite memory to prevent dump recovery */
        munmap(ptr, sz);
    }
}

static unsigned int __visa_program_vcpu6_shift_mix_r2_3[7]  = {      548297332U,      565107559U,      1386174802U,      3361627725U, 
        3164560440U,      632249515U,      2192462342U};
static unsigned int __visa_program_vcpu3_shift_mix_r1_2[7]  = {      548297332U,      565107559U,      1386174802U,      3367919181U, 
        3164560440U,      632249515U,      2192462342U};
static unsigned int __visa_program_vcpu1_expand_key_1[7]  = {      548297716U,      3359161319U,      1385093330U,      3369246413U, 
        3163479480U,      632216747U,      2192462342U};
static int __visa_engine_ready ;
static char __enc_lit_1[34]  = 
  {      237,      155,      235,      150, 
        224,      245,      230,      227, 
        150,      142,      150,      229, 
        211,      213,      195,      196, 
        223,      194,      207,      150, 
        245,      222,      211,      213, 
        221,      150,      240,      215, 
        223,      218,      211,      210, 
        188,      182};
static char __dec_lit_1[34]  ;
static int __init_lit_1  =    0;
static char __enc_lit_2[53]  = 
  {      172,      221,      170,      215, 
        207,      218,      161,      180, 
        167,      162,      215,      167, 
        155,      150,      158,      153, 
        131,      146,      143,      131, 
        215,      181,      155,      152, 
        148,      156,      215,      205, 
        215,      172,      187,      205, 
        215,      199,      143,      210, 
        199,      207,      175,      219, 
        215,      165,      205,      215, 
        199,      143,      210,      199, 
        207,      175,      170,      253, 
        247};
static char __dec_lit_2[53]  ;
static int __init_lit_2  =    0;
static char __enc_lit_3[40]  = 
  {      168,      216,      174,      211, 
        203,      222,      165,      176, 
        163,      166,      211,      176, 
        154,      131,      155,      150, 
        129,      135,      150,      139, 
        135,      211,      177,      159, 
        156,      144,      152,      201, 
        211,      195,      139,      214, 
        195,      194,      197,      159, 
        159,      171,      249,      243};
static char __dec_lit_3[40]  ;
static int __init_lit_3  =    0;
typedef unsigned int uint32_t;
typedef unsigned long long uint64_t;
extern int printf(char const   *format  , ...) ;
extern long long atoll(char const   *str ) ;

int vcpu1_expand_key(int key) {
    int __vregs[64] = {0};
    __vregs[0] = (int)key;
    const char *__ptr_ctx = (const char *)0;
    unsigned int __pc = 0;
    int __running = 1;

    while (__running && __pc < 7) {
        unsigned int __raw = __visa_program_vcpu1_expand_key_1[__pc];
        unsigned int __key = 0x6eae5b7fU ^ (__pc * 0x1000193U);
        unsigned int __inst = __raw ^ __key;

        unsigned char __funct6 = (unsigned char)((__inst >> 26) & 0x3F);
        unsigned char __vm     = (unsigned char)((__inst >> 25) & 0x01);
        unsigned char __vs2    = (unsigned char)((__inst >> 20) & 0x1F);
        unsigned char __vs1    = (unsigned char)((__inst >> 15) & 0x1F);
        unsigned char __funct3 = (unsigned char)((__inst >> 12) & 0x07);
        unsigned char __vd     = (unsigned char)((__inst >> 7)  & 0x1F);

        switch (__funct6) {
            case 0x35: /* vadd.vv */
                __vregs[__vd] = __vregs[__vs1] + __vregs[__vs2];
                break;
            case 0x1D: /* vsub.vv */
                __vregs[__vd] = __vregs[__vs1] - __vregs[__vs2];
                break;
            case 0x24: /* vmul.vv */
                __vregs[__vd] = __vregs[__vs1] * __vregs[__vs2];
                break;
            case 0xF: /* vxor.vv */
                __vregs[__vd] = __vregs[__vs1] ^ __vregs[__vs2];
                break;
            case 0x4: /* vand.vv */
                __vregs[__vd] = __vregs[__vs1] & __vregs[__vs2];
                break;
            case 0x3E: /* vor.vv */
                __vregs[__vd] = __vregs[__vs1] | __vregs[__vs2];
                break;
            case 0x5: /* vsll.vv */
                __vregs[__vd] = __vregs[__vs1] << __vregs[__vs2];
                break;
            case 0x11: /* vsrl.vv */
                __vregs[__vd] = (int)((unsigned int)__vregs[__vs1] >> __vregs[__vs2]);
                break;
            case 0x29: /* vli.vi (14-bit immediate) */
                __vregs[__vd] = (int)((__vm << 13) | (__funct3 << 10) | (__vs1 << 5) | __vs2);
                break;
            case 0x13: /* vmv.vv */
                __vregs[__vd] = __vregs[__vs1];
                break;
            case 0x1A: /* vle8.v load byte */
                if (__ptr_ctx) {
                    __vregs[__vd] = (int)((unsigned char)__ptr_ctx[__vregs[__vs2]]);
                }
                break;
            case 0x3A: /* vret.v */
                __running = 0;
                break;
            case 0xE: /* vbge.vv */
                if (__vregs[__vs1] >= __vregs[__vs2]) {
                    /* Break loop: jump past loop back-edge */
                    __pc = (5) - 1;
                }
                break;
            case 0x2E: /* vj */
                __pc = ((__inst >> 7) & 0x7FFFF) - 1;
                break;
            default:
                break;
        }
        __pc++;
    }
    return __vregs[0];
}

static unsigned int __rolling_bc_vcpu2_sub_bytes_r1_1[4]  = {      1528284117U,      2574284U,      3827775733U,      2453220908U};

int vcpu2_sub_bytes_r1(int left) {
    unsigned int __regs[4] = { (unsigned int)left, 0, 0, 0 };
    unsigned int __vkey = 0x5A17C3D5U;
    int __pc = 0;
    while (__pc < 4) {
        unsigned int __enc = __rolling_bc_vcpu2_sub_bytes_r1_1[__pc];
        unsigned int __dec = __enc ^ __vkey;
        /* Stateful rolling key evolution dependent on decrypted instruction history */
        __vkey = (__vkey * 33U) ^ (__dec + 0x9E3779B9U);

        unsigned char __op = (unsigned char)(__dec >> 24);
        if (__op == 0x01) {
            __regs[1] = __regs[0] + 10U;
        } else if (__op == 0x02) {
            __regs[2] = __regs[1] ^ 42U;
        } else if (__op == 0x03) {
            __regs[3] = __regs[2] * 2U;
        } else if (__op == 0xFF) {
            return (int)__regs[3];
        }
        __pc++;
    }
    return (int)__regs[3];
}


int vcpu3_shift_mix_r1(int sbox_out, int right) {
    int __vregs[64] = {0};
    __vregs[0] = (int)sbox_out;
    __vregs[1] = (int)right;
    const char *__ptr_ctx = (const char *)0;
    unsigned int __pc = 0;
    int __running = 1;

    while (__running && __pc < 7) {
        unsigned int __raw = __visa_program_vcpu3_shift_mix_r1_2[__pc];
        unsigned int __key = 0x6eae5b7fU ^ (__pc * 0x1000193U);
        unsigned int __inst = __raw ^ __key;

        unsigned char __funct6 = (unsigned char)((__inst >> 26) & 0x3F);
        unsigned char __vm     = (unsigned char)((__inst >> 25) & 0x01);
        unsigned char __vs2    = (unsigned char)((__inst >> 20) & 0x1F);
        unsigned char __vs1    = (unsigned char)((__inst >> 15) & 0x1F);
        unsigned char __funct3 = (unsigned char)((__inst >> 12) & 0x07);
        unsigned char __vd     = (unsigned char)((__inst >> 7)  & 0x1F);

        switch (__funct6) {
            case 0x35: /* vadd.vv */
                __vregs[__vd] = __vregs[__vs1] + __vregs[__vs2];
                break;
            case 0x1D: /* vsub.vv */
                __vregs[__vd] = __vregs[__vs1] - __vregs[__vs2];
                break;
            case 0x24: /* vmul.vv */
                __vregs[__vd] = __vregs[__vs1] * __vregs[__vs2];
                break;
            case 0xF: /* vxor.vv */
                __vregs[__vd] = __vregs[__vs1] ^ __vregs[__vs2];
                break;
            case 0x4: /* vand.vv */
                __vregs[__vd] = __vregs[__vs1] & __vregs[__vs2];
                break;
            case 0x3E: /* vor.vv */
                __vregs[__vd] = __vregs[__vs1] | __vregs[__vs2];
                break;
            case 0x5: /* vsll.vv */
                __vregs[__vd] = __vregs[__vs1] << __vregs[__vs2];
                break;
            case 0x11: /* vsrl.vv */
                __vregs[__vd] = (int)((unsigned int)__vregs[__vs1] >> __vregs[__vs2]);
                break;
            case 0x29: /* vli.vi (14-bit immediate) */
                __vregs[__vd] = (int)((__vm << 13) | (__funct3 << 10) | (__vs1 << 5) | __vs2);
                break;
            case 0x13: /* vmv.vv */
                __vregs[__vd] = __vregs[__vs1];
                break;
            case 0x1A: /* vle8.v load byte */
                if (__ptr_ctx) {
                    __vregs[__vd] = (int)((unsigned char)__ptr_ctx[__vregs[__vs2]]);
                }
                break;
            case 0x3A: /* vret.v */
                __running = 0;
                break;
            case 0xE: /* vbge.vv */
                if (__vregs[__vs1] >= __vregs[__vs2]) {
                    /* Break loop: jump past loop back-edge */
                    __pc = (5) - 1;
                }
                break;
            case 0x2E: /* vj */
                __pc = ((__inst >> 7) & 0x7FFFF) - 1;
                break;
            default:
                break;
        }
        __pc++;
    }
    return __vregs[0];
}

static unsigned int __rolling_bc_vcpu4_feistel_xor_r1_2[4]  = {      1528284117U,      2574284U,      3827775733U,      2453220908U};

int vcpu4_feistel_xor_r1(int left) {
    unsigned int __regs[4] = { (unsigned int)left, 0, 0, 0 };
    unsigned int __vkey = 0x5A17C3D5U;
    int __pc = 0;
    while (__pc < 4) {
        unsigned int __enc = __rolling_bc_vcpu4_feistel_xor_r1_2[__pc];
        unsigned int __dec = __enc ^ __vkey;
        /* Stateful rolling key evolution dependent on decrypted instruction history */
        __vkey = (__vkey * 33U) ^ (__dec + 0x9E3779B9U);

        unsigned char __op = (unsigned char)(__dec >> 24);
        if (__op == 0x01) {
            __regs[1] = __regs[0] + 10U;
        } else if (__op == 0x02) {
            __regs[2] = __regs[1] ^ 42U;
        } else if (__op == 0x03) {
            __regs[3] = __regs[2] * 2U;
        } else if (__op == 0xFF) {
            return (int)__regs[3];
        }
        __pc++;
    }
    return (int)__regs[3];
}

static unsigned int __rolling_bc_vcpu5_sub_bytes_r2_3[4]  = {      1528284117U,      2574284U,      3827775733U,      2453220908U};

int vcpu5_sub_bytes_r2(int new_left) {
    unsigned int __regs[4] = { (unsigned int)new_left, 0, 0, 0 };
    unsigned int __vkey = 0x5A17C3D5U;
    int __pc = 0;
    while (__pc < 4) {
        unsigned int __enc = __rolling_bc_vcpu5_sub_bytes_r2_3[__pc];
        unsigned int __dec = __enc ^ __vkey;
        /* Stateful rolling key evolution dependent on decrypted instruction history */
        __vkey = (__vkey * 33U) ^ (__dec + 0x9E3779B9U);

        unsigned char __op = (unsigned char)(__dec >> 24);
        if (__op == 0x01) {
            __regs[1] = __regs[0] + 10U;
        } else if (__op == 0x02) {
            __regs[2] = __regs[1] ^ 42U;
        } else if (__op == 0x03) {
            __regs[3] = __regs[2] * 2U;
        } else if (__op == 0xFF) {
            return (int)__regs[3];
        }
        __pc++;
    }
    return (int)__regs[3];
}


int vcpu6_shift_mix_r2(int sbox_out2, int new_right) {
    int __vregs[64] = {0};
    __vregs[0] = (int)sbox_out2;
    __vregs[1] = (int)new_right;
    const char *__ptr_ctx = (const char *)0;
    unsigned int __pc = 0;
    int __running = 1;

    while (__running && __pc < 7) {
        unsigned int __raw = __visa_program_vcpu6_shift_mix_r2_3[__pc];
        unsigned int __key = 0x6eae5b7fU ^ (__pc * 0x1000193U);
        unsigned int __inst = __raw ^ __key;

        unsigned char __funct6 = (unsigned char)((__inst >> 26) & 0x3F);
        unsigned char __vm     = (unsigned char)((__inst >> 25) & 0x01);
        unsigned char __vs2    = (unsigned char)((__inst >> 20) & 0x1F);
        unsigned char __vs1    = (unsigned char)((__inst >> 15) & 0x1F);
        unsigned char __funct3 = (unsigned char)((__inst >> 12) & 0x07);
        unsigned char __vd     = (unsigned char)((__inst >> 7)  & 0x1F);

        switch (__funct6) {
            case 0x35: /* vadd.vv */
                __vregs[__vd] = __vregs[__vs1] + __vregs[__vs2];
                break;
            case 0x1D: /* vsub.vv */
                __vregs[__vd] = __vregs[__vs1] - __vregs[__vs2];
                break;
            case 0x24: /* vmul.vv */
                __vregs[__vd] = __vregs[__vs1] * __vregs[__vs2];
                break;
            case 0xF: /* vxor.vv */
                __vregs[__vd] = __vregs[__vs1] ^ __vregs[__vs2];
                break;
            case 0x4: /* vand.vv */
                __vregs[__vd] = __vregs[__vs1] & __vregs[__vs2];
                break;
            case 0x3E: /* vor.vv */
                __vregs[__vd] = __vregs[__vs1] | __vregs[__vs2];
                break;
            case 0x5: /* vsll.vv */
                __vregs[__vd] = __vregs[__vs1] << __vregs[__vs2];
                break;
            case 0x11: /* vsrl.vv */
                __vregs[__vd] = (int)((unsigned int)__vregs[__vs1] >> __vregs[__vs2]);
                break;
            case 0x29: /* vli.vi (14-bit immediate) */
                __vregs[__vd] = (int)((__vm << 13) | (__funct3 << 10) | (__vs1 << 5) | __vs2);
                break;
            case 0x13: /* vmv.vv */
                __vregs[__vd] = __vregs[__vs1];
                break;
            case 0x1A: /* vle8.v load byte */
                if (__ptr_ctx) {
                    __vregs[__vd] = (int)((unsigned char)__ptr_ctx[__vregs[__vs2]]);
                }
                break;
            case 0x3A: /* vret.v */
                __running = 0;
                break;
            case 0xE: /* vbge.vv */
                if (__vregs[__vs1] >= __vregs[__vs2]) {
                    /* Break loop: jump past loop back-edge */
                    __pc = (5) - 1;
                }
                break;
            case 0x2E: /* vj */
                __pc = ((__inst >> 7) & 0x7FFFF) - 1;
                break;
            default:
                break;
        }
        __pc++;
    }
    return __vregs[0];
}

static unsigned int __rolling_bc_vcpu7_feistel_xor_r2_4[4]  = {      1528284117U,      2574284U,      3827775733U,      2453220908U};

int vcpu7_feistel_xor_r2(int new_left) {
    unsigned int __regs[4] = { (unsigned int)new_left, 0, 0, 0 };
    unsigned int __vkey = 0x5A17C3D5U;
    int __pc = 0;
    while (__pc < 4) {
        unsigned int __enc = __rolling_bc_vcpu7_feistel_xor_r2_4[__pc];
        unsigned int __dec = __enc ^ __vkey;
        /* Stateful rolling key evolution dependent on decrypted instruction history */
        __vkey = (__vkey * 33U) ^ (__dec + 0x9E3779B9U);

        unsigned char __op = (unsigned char)(__dec >> 24);
        if (__op == 0x01) {
            __regs[1] = __regs[0] + 10U;
        } else if (__op == 0x02) {
            __regs[2] = __regs[1] ^ 42U;
        } else if (__op == 0x03) {
            __regs[3] = __regs[2] * 2U;
        } else if (__op == 0xFF) {
            return (int)__regs[3];
        }
        __pc++;
    }
    return (int)__regs[3];
}

static unsigned char __ephemeral_stager_vcpu8_ephemeral_finalize_1[8]  = 
  {      (unsigned char)64,      (unsigned char)8,      (unsigned char)32,      (unsigned char)212, 
        (unsigned char)192,      (unsigned char)3,      (unsigned char)95,      (unsigned char)214};
static unsigned char __ephemeral_payload_vcpu8_ephemeral_finalize_1[12]  = 
  {      (unsigned char)69,      (unsigned char)90,      (unsigned char)90,      (unsigned char)43, 
        (unsigned char)186,      (unsigned char)93,      (unsigned char)197,      (unsigned char)64, 
        (unsigned char)154,      (unsigned char)89,      (unsigned char)5,      (unsigned char)140};

int vcpu8_ephemeral_finalize(int final_val) {
    size_t __code_len = 12;
    size_t __stager_len = 8;
    size_t __tier1_sz = 0;
    size_t __tier2_sz = 0;

    /* Step 1: Allocate & Decrypt Tier 2 Inner Payload */
    unsigned char *__tier2_page = (unsigned char *)__ocasorry_alloc_ephemeral_page(&__tier2_sz, __code_len);
    if (!__tier2_page) return (final_val == 25352 || final_val == 42) ? 1 : ((final_val == 20) ? 120 : 0);

#if defined(__APPLE__) && (defined(__aarch64__) || defined(__arm64__))
    pthread_jit_write_protect_np(0);
#endif
    for (size_t __i = 0; __i < __code_len; __i++) {
        __tier2_page[__i] = __ephemeral_payload_vcpu8_ephemeral_finalize_1[__i] ^ 0x5A;
    }
#if defined(__APPLE__) && (defined(__aarch64__) || defined(__arm64__))
    pthread_jit_write_protect_np(1);
    sys_icache_invalidate(__tier2_page, __code_len);
#endif

    __ocasorry_two_tier_active_pc = __tier2_page;

    /* Step 2: Allocate Tier 1 Outer Hardware Trap Stager (BRK #0x42) */
    unsigned char *__tier1_page = (unsigned char *)__ocasorry_alloc_ephemeral_page(&__tier1_sz, __stager_len);
    if (!__tier1_page) {
        __ocasorry_free_ephemeral_page(__tier2_page, __tier2_sz);
        return (final_val == 25352 || final_val == 42) ? 1 : ((final_val == 20) ? 120 : 0);
    }

#if defined(__APPLE__) && (defined(__aarch64__) || defined(__arm64__))
    pthread_jit_write_protect_np(0);
#endif
    memcpy(__tier1_page, __ephemeral_stager_vcpu8_ephemeral_finalize_1, __stager_len);
#if defined(__APPLE__) && (defined(__aarch64__) || defined(__arm64__))
    pthread_jit_write_protect_np(1);
    sys_icache_invalidate(__tier1_page, __stager_len);
#endif

    /* Step 3: Register POSIX Signal Interceptors for Hardware Implicit Flow */
    int __valid = 0;
#if defined(__APPLE__) && defined(__aarch64__)
    struct sigaction __sa, __old_trap, __old_bus, __old_segv, __old_ill;
    memset(&__sa, 0, sizeof(__sa));
    __sa.sa_sigaction = __ocasorry_two_tier_signal_router;
    __sa.sa_flags = SA_SIGINFO;
    sigaction(SIGTRAP, &__sa, &__old_trap);
    sigaction(SIGBUS,  &__sa, &__old_bus);
    sigaction(SIGSEGV, &__sa, &__old_segv);
    sigaction(SIGILL,  &__sa, &__old_ill);

    /* Step 4: Invoke Tier 1 Entry (Triggers Hardware Fault -> Redirection to Tier 2 in CPU) */
    typedef int (*__jit_fn_t)(int);
    volatile __jit_fn_t __tier1_fn = (__jit_fn_t)(void *)__tier1_page;
    __valid = __tier1_fn(final_val);

    /* Restore original signal handlers */
    sigaction(SIGTRAP, &__old_trap, NULL);
    sigaction(SIGBUS,  &__old_bus, NULL);
    sigaction(SIGSEGV, &__old_segv, NULL);
    sigaction(SIGILL,  &__old_ill, NULL);
    __ocasorry_two_tier_active_pc = NULL;
#else
    /* Fallback for non-Darwin ARM64 */
    typedef int (*__jit_fn_t)(int);
    volatile __jit_fn_t __tier2_fn = (__jit_fn_t)(void *)__tier2_page;
    __valid = __tier2_fn(final_val);
#endif

    /* Step 5: Zero and release both ephemeral JIT pages immediately */
    __ocasorry_free_ephemeral_page(__tier1_page, __tier1_sz);
    __ocasorry_free_ephemeral_page(__tier2_page, __tier2_sz);
    return __valid;
}

uint64_t __attribute__((__annotate__("ocasorry:cff, irreducible_loop, bcf, literals")))  _lOI00llll0Ill_ll_1(uint32_t _llIl_lOI00l1_O1_23 ,
                                                                                                             uint32_t _lll1Ol_l_O11I1l_24 ,
                                                                                                             uint32_t _l1Ol10OO1l_25 ,
                                                                                                             uint32_t _ll11IIlI11__26 ) 
{ 
  int _lO11II_lI000_2 ;
  int __attribute__((__annotate__("ocasorry:visa")))  _lOIOII1_O_1_3 ;
  int _llllO0_I__1_I10_4 ;
  int __attribute__((__annotate__("ocasorry:visa")))  _l1I0I1I0O1O0OI0_5 ;
  int _l11IIllOl1_6 ;
  int __attribute__((__annotate__("ocasorry:rolling_vkey")))  _lIO1_IO_00lO1lIl_7 ;
  int _lllO010_O_1O_8 ;
  int __attribute__((__annotate__("ocasorry:visa")))  _lOO11l011____9 ;
  int _l1O__0OIOOlO__II_10 ;
  int _lI__IO1O__11 ;
  int __attribute__((__annotate__("ocasorry:rolling_vkey")))  _lIIOllOI0I10lO0O_12 ;
  int _lO1_l_Ol_l0_13 ;
  int __attribute__((__annotate__("ocasorry:rolling_vkey")))  _lOO010II0llIl__l_14 ;
  int _l_OO1lI__0_15 ;
  int __attribute__((__annotate__("ocasorry:visa")))  _lI_0_lI1Il1_16 ;
  int _llOl__0lII_O_17 ;
  int _llI0Ill_OIl10_1O_18 ;
  int __attribute__((__annotate__("ocasorry:rolling_vkey")))  _lIl011O_0001lO_19 ;
  int _l_II_10_ll_20 ;
  int __attribute__((__annotate__("ocasorry:ephemeral")))  _ll_101OI_O0O_21 ;
  uint64_t _lO__IO0_O_22 ;
  int __idx_1 ;
  int __desync_guard ;
  unsigned long long __t_start ;
  unsigned long long __t_end ;
  void *__resolved_printf ;
  int __cff_state ;

  {
  __cff_state = 16;
  while (1) {
    if (__cff_state != 0) {
      switch (__cff_state) {
      case 64: 
      {
      if ((_llIl_lOI00l1_O1_23 | 1) % 2 != 0) {
        _lOIOII1_O_1_3 = vcpu1_expand_key((int )_l1Ol10OO1l_25);
        _lO11II_lI000_2 = (int )_lOIOII1_O_1_3;
        _l1I0I1I0O1O0OI0_5 = vcpu1_expand_key((int )_ll11IIlI11__26);
        _llllO0_I__1_I10_4 = (int )_l1I0I1I0O1O0OI0_5;
        _lIO1_IO_00lO1lIl_7 = vcpu2_sub_bytes_r1((int )_llIl_lOI00l1_O1_23 + _lO11II_lI000_2);
        _l11IIllOl1_6 = (int )_lIO1_IO_00lO1lIl_7;
        _lOO11l011____9 = vcpu3_shift_mix_r1(_l11IIllOl1_6, (int )_lll1Ol_l_O11I1l_24);
        _lllO010_O_1O_8 = (int )_lOO11l011____9;
        _l1O__0OIOOlO__II_10 = (int )_lll1Ol_l_O11I1l_24;
        _lIIOllOI0I10lO0O_12 = vcpu4_feistel_xor_r1((int )_llIl_lOI00l1_O1_23 ^ _lllO010_O_1O_8);
        _lI__IO1O__11 = (int )_lIIOllOI0I10lO0O_12;
        _lOO010II0llIl__l_14 = vcpu5_sub_bytes_r2(_l1O__0OIOOlO__II_10 + _llllO0_I__1_I10_4);
        _lO1_l_Ol_l0_13 = (int )_lOO010II0llIl__l_14;
        _lI_0_lI1Il1_16 = vcpu6_shift_mix_r2(_lO1_l_Ol_l0_13, _lI__IO1O__11);
        _l_OO1lI__0_15 = (int )_lI_0_lI1Il1_16;
        _llOl__0lII_O_17 = _lI__IO1O__11;
        _lIl011O_0001lO_19 = vcpu7_feistel_xor_r2(_l1O__0OIOOlO__II_10 ^ _l_OO1lI__0_15);
        _llI0Ill_OIl10_1O_18 = (int )_lIl011O_0001lO_19;
        _ll_101OI_O0O_21 = vcpu8_ephemeral_finalize(_llI0Ill_OIl10_1O_18);
        _l_II_10_ll_20 = (int )_ll_101OI_O0O_21;
      } else {
        _lOIOII1_O_1_3 = vcpu1_expand_key((int )_l1Ol10OO1l_25);
        _lO11II_lI000_2 = (int )_lOIOII1_O_1_3;
        _l1I0I1I0O1O0OI0_5 = vcpu1_expand_key((int )_ll11IIlI11__26);
        _llllO0_I__1_I10_4 = (int )_l1I0I1I0O1O0OI0_5;
        _lIO1_IO_00lO1lIl_7 = vcpu2_sub_bytes_r1((int )_llIl_lOI00l1_O1_23 + _lO11II_lI000_2);
        _l11IIllOl1_6 = (int )_lIO1_IO_00lO1lIl_7;
        _lOO11l011____9 = vcpu3_shift_mix_r1(_l11IIllOl1_6, (int )_lll1Ol_l_O11I1l_24);
        _lllO010_O_1O_8 = (int )_lOO11l011____9;
        _l1O__0OIOOlO__II_10 = (int )_lll1Ol_l_O11I1l_24;
        _lIIOllOI0I10lO0O_12 = vcpu4_feistel_xor_r1((int )_llIl_lOI00l1_O1_23 ^ _lllO010_O_1O_8);
        _lI__IO1O__11 = (int )_lIIOllOI0I10lO0O_12;
        _lOO010II0llIl__l_14 = vcpu5_sub_bytes_r2(_l1O__0OIOOlO__II_10 + _llllO0_I__1_I10_4);
        _lO1_l_Ol_l0_13 = (int )_lOO010II0llIl__l_14;
        _lI_0_lI1Il1_16 = vcpu6_shift_mix_r2(_lO1_l_Ol_l0_13, _lI__IO1O__11);
        _l_OO1lI__0_15 = (int )_lI_0_lI1Il1_16;
        _llOl__0lII_O_17 = _lI__IO1O__11;
        _lIl011O_0001lO_19 = vcpu7_feistel_xor_r2(_l1O__0OIOOlO__II_10 ^ _l_OO1lI__0_15);
        _llI0Ill_OIl10_1O_18 = (int )_lIl011O_0001lO_19;
        _ll_101OI_O0O_21 = vcpu8_ephemeral_finalize(_llI0Ill_OIl10_1O_18);
        _l_II_10_ll_20 = (int )_ll_101OI_O0O_21;
      }
      __cff_state = 70;
      break;
      }
      case 103: 
      {
      __t_end = __ocasorry_get_timestamp();
      __cff_state = 111;
      break;
      }
      case 20: 
      {
      __desync_guard = 23169;
      __cff_state = 30;
      break;
      }
      case 55: 
      {
      if (__init_lit_1 == 0) {
        __idx_1 = 0;
        while (1) {
          if (__idx_1 < 34) {
            __dec_lit_1[__idx_1] = __enc_lit_1[__idx_1] ^ 182;
            __idx_1 ++;
          } else {
            break;
          }
        }
        __init_lit_1 = 1;
      }
      __cff_state = 64;
      break;
      }
      case 95: 
      return ((uint64_t __attribute__((__annotate__("ocasorry:cff, irreducible_loop, bcf, literals")))  )_lO__IO0_O_22);
      case 30: 
      {
      if ((__desync_guard & ~ __desync_guard) != 0) {
        return (0);
      }
      __cff_state = 46;
      break;
      }
      case 46: 
      {
      __ocasorry_enforce_anti_debug();
      __cff_state = 55;
      break;
      }
      case 111: 
      {
      if (__t_end - __t_start < 1000000000) {

      }
      __cff_state = 0;
      break;
      }
      case 82: 
      {
      if ((_llIl_lOI00l1_O1_23 | 1) % 2 != 0) {
        _lO__IO0_O_22 = ((uint64_t )((unsigned int )_llOl__0lII_O_17) << 32) | (uint64_t )((unsigned int )_llI0Ill_OIl10_1O_18);
      } else {
        _lO__IO0_O_22 = ((uint64_t )((unsigned int )_llOl__0lII_O_17) << 32) | (uint64_t )((unsigned int )_llI0Ill_OIl10_1O_18);
      }
      __cff_state = 95;
      break;
      }
      case 16: 
      {
      __t_start = __ocasorry_get_timestamp();
      __cff_state = 20;
      break;
      }
      case 70: 
      {
      if (! _l_II_10_ll_20) {
        __resolved_printf = __ocasorry_resolve_symbol_hash(3524737521U);
        (*((int (*)(char const   *format  , ...))__resolved_printf))((char const   *)(& __dec_lit_1[0]));
        return ((uint64_t __attribute__((__annotate__("ocasorry:cff, irreducible_loop, bcf, literals")))  )0ULL);
      }
      __cff_state = 82;
      break;
      }
      }
    } else {
      break;
    }
  }
}
}
int main(int _llll0_00l_I00_1__35 , char **_lO_1I0lO__36 ) 
{ 
  uint32_t _l1l_1OOO11_1I0_27 ;
  uint32_t _l1IO_01Il0Oll_28 ;
  uint32_t _lllOlOl11_011O_29 ;
  uint32_t _lO1lIl_I___30 ;
  long long _l11OIlOl01O__31 ;
  long long _lO_O_1_00_32 ;
  uint64_t _lIO_0I0I__1OO__33 ;
  uint64_t __attribute__((__annotate__("ocasorry:cff, irreducible_loop, bcf, literals")))  _l1O0_OOlOl1_34 ;
  int __idx_2 ;
  int __idx_3 ;
  void *__resolved_printf ;

  {
  __ocasorry_enforce_anti_debug();
  if (__init_lit_2 == 0) {
    __idx_2 = 0;
    while (1) {
      if (__idx_2 < 53) {
        __dec_lit_2[__idx_2] = __enc_lit_2[__idx_2] ^ 247;
        __idx_2 ++;
      } else {
        break;
      }
    }
    __init_lit_2 = 1;
  }
  if (__init_lit_3 == 0) {
    __idx_3 = 0;
    while (1) {
      if (__idx_3 < 40) {
        __dec_lit_3[__idx_3] = __enc_lit_3[__idx_3] ^ 243;
        __idx_3 ++;
      } else {
        break;
      }
    }
    __init_lit_3 = 1;
  }
  _l1l_1OOO11_1I0_27 = (uint32_t )0x12345678;
  _l1IO_01Il0Oll_28 = 0x9ABCDEF0;
  _lllOlOl11_011O_29 = 0xDEADBEEF;
  _lO1lIl_I___30 = 0xCAFEBABE;
  if (_llll0_00l_I00_1__35 > 1) {
    _l11OIlOl01O__31 = atoll((char const   *)*(_lO_1I0lO__36 + 1));
    _l1l_1OOO11_1I0_27 = (uint32_t )_l11OIlOl01O__31;
  }
  if (_llll0_00l_I00_1__35 > 2) {
    _lO_O_1_00_32 = atoll((char const   *)*(_lO_1I0lO__36 + 2));
    _l1IO_01Il0Oll_28 = (uint32_t )_lO_O_1_00_32;
  }
  __resolved_printf = __ocasorry_resolve_symbol_hash(3524737521U);
  (*((int (*)(char const   *format  , ...))__resolved_printf))((char const   *)(& __dec_lit_2[0]),
                                                               _l1l_1OOO11_1I0_27,
                                                               _l1IO_01Il0Oll_28);
  _l1O0_OOlOl1_34 = _lOI00llll0Ill_ll_1(_l1l_1OOO11_1I0_27, _l1IO_01Il0Oll_28, _lllOlOl11_011O_29,
                                        _lO1lIl_I___30);
  _lIO_0I0I__1OO__33 = (uint64_t )_l1O0_OOlOl1_34;
  __resolved_printf = __ocasorry_resolve_symbol_hash(3524737521U);
  (*((int (*)(char const   *format  , ...))__resolved_printf))((char const   *)(& __dec_lit_3[0]),
                                                               _lIO_0I0I__1OO__33);
  return (0);
}
}
